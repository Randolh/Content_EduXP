...
Bienvenido al curso de **Git y GitHub**. En esta primera lección aprenderás qué es un sistema de control de versiones distribuido, cómo instalar y configurar Git en tu equipo y cómo mover archivos entre los tres estados fundamentales de un repositorio local.
...
**Objetivo**: Inicializar un repositorio local, crear un archivo, añadirlo al Staging Area y realizar tu primer commit.

**Instrucciones**:
1. Crea una carpeta llamada `mi-primer-repo` e ingresa a ella desde la terminal.
2. Inicializa el repositorio con `git init`.
3. Crea un archivo `index.html` con un texto simple.
4. Consulta el estado con `git status`.
5. Agrega el archivo al staging area y confirma los cambios con un commit.
...
# 4. Verificar estado (aparecerá como Untracked)
git status

# 5. Mover al Staging Area
git add index.html

# 6. Confirmar cambios con un mensaje descriptivo
